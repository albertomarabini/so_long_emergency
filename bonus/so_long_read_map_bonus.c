...
/*   Updated: 2023/08/02 07:41:56 by amarabin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	validate_map(t_game *game)
{
	...
	vill_c = 0;
	game->a_vills = (t_point **)malloc(sizeof(t_point *) * (game->vills + 1));
	r = 0;
	while (r < game->map_h)
	{
		c = 0;
		while (c < game->map_w)
		{
			if ((r > 0 || r < game->map_h - 1) && game->map[r][c] == 'V')
				game->a_vills[vill_c++] = create_point(r, c, 0);
			c++;
		}
		r++;
	}
